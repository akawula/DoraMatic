...
func (q *Queries) DeletePullRequestFiles(ctx context.Context, pullRequestID string) error {
	_, err := q.db.Exec(ctx, deletePullRequestFiles, pullRequestID)
	return err
}
...
type GetForeignPRMetricsByTeamRow struct {
	...
	ClosedForeignPrs      int32   `db:"closed_foreign_prs"`
...
const getPRsWithIncompleteFiles = `-- name: GetPRsWithIncompleteFiles :many
...
type GetPRsWithIncompleteFilesRow struct {
	ID              string             `db:"id"`
	Url             sql.NullString     `db:"url"`
	Title           sql.NullString     `db:"title"`
	RepositoryName  pgtype.Text        `db:"repository_name"`
	...
	Author          pgtype.Text        `db:"author"`
	State           pgtype.Text        `db:"state"`
	ChangedFiles    pgtype.Int4        `db:"changed_files"`
	Additions       pgtype.Int4        `db:"additions"`
	Deletions       pgtype.Int4        `db:"deletions"`
	CreatedAt       time.Time          `db:"created_at"`
	MergedAt        pgtype.Timestamptz `db:"merged_at"`
...
func (q *Queries) GetPRsWithIncompleteFiles(ctx context.Context) ([]GetPRsWithIncompleteFilesRow, error) {
	rows, err := q.db.Query(ctx, getPRsWithIncompleteFiles)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetPRsWithIncompleteFilesRow{}
	for rows.Next() {
		var i GetPRsWithIncompleteFilesRow
		if err := rows.Scan(
			&i.ID,
			&i.Url,
			&i.Title,
...
const getPullRequestFiles = `-- name: GetPullRequestFiles :many
...
func (q *Queries) GetPullRequestFiles(ctx context.Context, pullRequestID string) ([]PullRequestFile, error) {
	rows, err := q.db.Query(ctx, getPullRequestFiles, pullRequestID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const getTeamGeneratedCodeStatsByDateRange = `-- name: GetTeamGeneratedCodeStatsByDateRange :one
...
WHERE t.team = $1
  AND ($2::text[] IS NULL OR p.author = ANY($2::text[]))
  AND p.state = 'MERGED'
  AND p.merged_at >= $3::timestamptz
  AND p.merged_at <= $4::timestamptz
`

type GetTeamGeneratedCodeStatsByDateRangeParams struct {
	TeamName  string    `db:"team_name"`
	Members   []string  `db:"members"`
	StartDate time.Time `db:"start_date"`
	EndDate   time.Time `db:"end_date"`
...
type GetTeamGeneratedCodeStatsByDateRangeRow struct {
	MergedCount            int32 `db:"merged_count"`
	TotalAdditions         int64 `db:"total_additions"`
	TotalDeletions         int64 `db:"total_deletions"`
	GeneratedAdditions     int64 `db:"generated_additions"`
	GeneratedDeletions     int64 `db:"generated_deletions"`
	HumanAdditions         int64 `db:"human_additions"`
	HumanDeletions         int64 `db:"human_deletions"`
	TotalFilesChanged      int64 `db:"total_files_changed"`
	PrsWithIncompleteFiles int32 `db:"prs_with_incomplete_files"`
}

// Team Statistics with Generated Code Metrics --
func (q *Queries) GetTeamGeneratedCodeStatsByDateRange(ctx context.Context, arg GetTeamGeneratedCodeStatsByDateRangeParams) (GetTeamGeneratedCodeStatsByDateRangeRow, error) {
	row := q.db.QueryRow(ctx, getTeamGeneratedCodeStatsByDateRange,
		arg.TeamName,
		arg.Members,
		arg.StartDate,
		arg.EndDate,
	)
	var i GetTeamGeneratedCodeStatsByDateRangeRow
	err := row.Scan(
		&i.MergedCount,
		&i.TotalAdditions,
		&i.TotalDeletions,
		&i.GeneratedAdditions,
		&i.GeneratedDeletions,
		&i.HumanAdditions,
		&i.HumanDeletions,
		&i.TotalFilesChanged,
		&i.PrsWithIncompleteFiles,
	)
	return i, err
}
...
SELECT
    ...
    COUNT(CASE WHEN p.state = 'CLOSED' AND p.closed_at >= $1::timestamptz AND p.closed_at <= $2::timestamptz THEN 1 END)::int AS closed_count,
    -- Calculate average time from creation to close for closed (not merged) PRs
    COALESCE(AVG(
        CASE
            WHEN p.state = 'CLOSED' AND p.closed_at IS NOT NULL AND p.closed_at >= $1::timestamptz AND p.closed_at <= $2::timestamptz
...
WHERE t.team = $3
  ...
  AND (
       ...
       (p.state = 'CLOSED' AND p.closed_at >= $1::timestamptz AND p.closed_at <= $2::timestamptz)
      )
`
...
type GetTeamPullRequestStatsByDateRangeRow struct {
	...
	AvgTimeToCloseSeconds                 float64 `db:"avg_time_to_close_seconds"`
...
const getTopGeneratedFilePatterns = `-- name: GetTopGeneratedFilePatterns :many
SELECT
...
type GetTopGeneratedFilePatternsParams struct {
	TeamName  string    `db:"team_name"`
	StartDate time.Time `db:"start_date"`
	EndDate   time.Time `db:"end_date"`
}

type GetTopGeneratedFilePatternsRow struct {
...
func (q *Queries) GetTopGeneratedFilePatterns(ctx context.Context, arg GetTopGeneratedFilePatternsParams) ([]GetTopGeneratedFilePatternsRow, error) {
	rows, err := q.db.Query(ctx, getTopGeneratedFilePatterns, arg.TeamName, arg.StartDate, arg.EndDate)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetTopGeneratedFilePatternsRow{}
	for rows.Next() {
		var i GetTopGeneratedFilePatternsRow
		if err := rows.Scan(
			&i.Pattern,
			&i.FileCount,
			&i.TotalAdditions,
...
const updatePRGeneratedStats = `-- name: UpdatePRGeneratedStats :exec
UPDATE prs SET
    changed_files = $2,
...
type UpdatePRGeneratedStatsParams struct {
	ID                 string      `db:"id"`
	ChangedFiles       pgtype.Int4 `db:"changed_files"`
	GeneratedAdditions pgtype.Int4 `db:"generated_additions"`
	GeneratedDeletions pgtype.Int4 `db:"generated_deletions"`
	FilesComplete      pgtype.Bool `db:"files_complete"`
}

func (q *Queries) UpdatePRGeneratedStats(ctx context.Context, arg UpdatePRGeneratedStatsParams) error {
	_, err := q.db.Exec(ctx, updatePRGeneratedStats,
		arg.ID,
		arg.ChangedFiles,
		arg.GeneratedAdditions,
		arg.GeneratedDeletions,
		arg.FilesComplete,
	)
	return err
}
...
const upsertPullRequestFile = `-- name: UpsertPullRequestFile :exec
...
type UpsertPullRequestFileParams struct {
	PullRequestID string      `db:"pull_request_id"`
	FilePath      string      `db:"file_path"`
	Additions     int32       `db:"additions"`
	Deletions     int32       `db:"deletions"`
	Status        pgtype.Text `db:"status"`
	IsGenerated   bool        `db:"is_generated"`
}

// Pull Request Files (Generated Code Tracking) --
func (q *Queries) UpsertPullRequestFile(ctx context.Context, arg UpsertPullRequestFileParams) error {
	_, err := q.db.Exec(ctx, upsertPullRequestFile,
		arg.PullRequestID,
		arg.FilePath,
		arg.Additions,
		arg.Deletions,
